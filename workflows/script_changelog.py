...
def get_submodule_commits(old_hash: str, new_hash: str) -> list[str]:
    ...
    # Drop commits already present in old_hash's history: on divergent release
    # branches the same change is re-landed with an edited patch (new patch-id),
    # so it survives --cherry-pick but is not actually new to this range.
    #
    # The de-dup key is (subject, touched-file set) rather than subject alone.
    # Subject-only is too coarse: generic subjects like "fix: update manifests"
    # or "chore: bump version" recur across unrelated commits, so a genuinely-new
    # change reusing a common subject would be dropped. Pairing the subject with
    # the exact set of files it touches makes the key specific enough that only a
    # true re-land (same change, same files) matches.
    seen = subprocess.run(
        ['git', '-C', SUBMODULE_PATH, 'log', '--format=%x00%s', '--name-only', old_hash],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    if seen.returncode != 0:
        logger.warning(
            f"Failed to read subject/file history for {old_hash[:7]} "
            f"({seen.stderr.decode().strip()}); skipping re-land de-duplication "
            f"— already-shipped API commits may reappear in the release notes."
        )
        seen_keys = set()
    else:
        # Each commit's output begins with a NUL (%x00) then its subject, followed
        # by the --name-only file list; split on NUL to recover per-commit records.
        seen_keys = set()
        for record in seen.stdout.decode().split('\x00'):
            lines = record.splitlines()
            if not lines:
                continue
            subject = lines[0]
            files = frozenset(l for l in lines[1:] if l.strip())
            seen_keys.add((subject, files))

    filtered = []
    for sha in commits:
        msg = get_submodule_commit_message(sha)
        subj = msg.splitlines()[0] if msg else ""
        files = frozenset(get_submodule_commit_files(sha))
        if subj and (subj, files) in seen_keys:
            logger.info(f"API commit {sha[:7]} '{subj}' already present before range (same subject+files re-land); skipping.")
            continue
        filtered.append(sha)
    return filtered
