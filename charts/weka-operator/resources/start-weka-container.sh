...
if [[ "$MODE" == "dist" ]]; then
  weka version prepare $(weka version current) || weka local setup container \
    --name dist \
    --net udp \
    --base-port "${PORT}" \
    "${APPEND_SETUP_COMMAND}"
  copy_drivers
  wait_for_shutdown
  exit 0
fi
...
log_message INFO weka local setup container --name ${NAME} --no-start --net ${NETWORK_DEVICE} --cores ${CORES} ${MODE_SELECTOR} --base-port ${PORT} --memory ${MEMORY} --core-ids ${CORE_IDS} ${JOIN_IPS_BLOCK} "${APPEND_SETUP_COMMAND}"
weka local setup container --name ${NAME} --no-start --net ${NETWORK_DEVICE} --cores ${CORES} ${MODE_SELECTOR} --base-port ${PORT} --memory ${MEMORY} --core-ids ${CORE_IDS} ${JOIN_IPS_BLOCK} "${APPEND_SETUP_COMMAND}" 2> >(log_pipe >&2) | log_pipe
