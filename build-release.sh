...
docker buildx build --platform linux/amd64 --tag quay.io/weka.io/weka-operator:v$VERSION --push -f image.Dockerfile . || { echo "docker build failed, ensure login and re-run whole flow"; exit 1; }
