...
async def ensure_daemon(command, alias=""):
    ...
    def ignore_signals():
        ...
        # signal.signal(signal.SIGUSR2, signal.SIGTERM)
    ...
    process = await asyncio.create_subprocess_shell(command, preexec_fn=ignore_signals)
...
async def main():
    logging.info("main func")
    agent_cmd = f"sleep 10001"
    await ensure_daemon(agent_cmd, alias="agent")
    await ensure_daemon(f"sleep 10002")
...
async def stop_daemon(process):
    ...
    for k, v in list(processes.items()):
        if v == process:
            del processes[k]
    logging.info("waiting for process %s to finish(2)", process)
...
async def shutdown():
    ...
    await run_command("echo stopping; sleep 5; echo stopped")
    logging.info("finished stopping weka container")

    for process in list(processes.values()):
        logging.info("awaiting for process %s", process)
        await stop_daemon(process)
        logging.info("done awaiting for process %s", process)

    tasks = [t for t in asyncio.all_tasks(loop) if t is not asyncio.current_task(loop)]
    [task.cancel() for task in tasks]

    logging.info("cancelled everything (pre loop stop)")
    loop.stop()
    logging.info("cancelled everything (post loop stop)")
...
try:
    logging.info("running until completed")
    loop.run_forever()
    logging.info("done running until completed")
except RuntimeError:
