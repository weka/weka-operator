...
func getFieldDescription(field *ast.Field, file *ast.File) string {
	// First check field's own comment
	if field.Comment != nil {
		for _, comment := range field.Comment.List {
			text := strings.TrimPrefix(comment.Text, "//")
			text = strings.TrimSpace(text)
			if !strings.HasPrefix(text, "+") && text != "" { // Skip kubebuilder annotations
				return text
			}
		}
	}
	...
	for _, commentGroup := range file.Comments {
		for _, comment := range commentGroup.List {
			// Check if comment is right before this field (within a few lines)
			commentEnd := comment.End()
			if commentEnd < fieldPos && int(fieldPos-commentEnd) < 200 { // rough line distance
				text := strings.TrimPrefix(comment.Text, "//")
				text = strings.TrimSpace(text)
				if !strings.HasPrefix(text, "+") && text != "" { // Skip kubebuilder annotations
					return text
