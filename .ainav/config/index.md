...
- Removed-node backend cleanup (`CLEANUP_REMOVED_NODES`, Helm `cleanupRemovedNodes`) — tri-state `false`/`true`/`auto`, **default `auto`**. `false` = never delete a backend WekaContainer when its affinity node leaves the cluster; `true` = delete immediately; `auto` = hold the container in `Stale` status for a grace period, then delete only if the node is still gone. Grace = **30m on managed cloud (aws/oci)**, **24h otherwise** (`funcs_handle_node_statuses.go` `nodeRemovalGracePeriodManagedCloud`/`nodeRemovalGracePeriod`). Parser `getCleanupRemovedNodesMode()` in `env.go` treats unset/empty as `auto` and fails **closed to `false`** on an unrecognized value. Grace logic: `deleteIfNoNode` → `handleBackendNodeRemovalGrace` (`funcs_handle_node_statuses.go`).
