...
// APIErrorSummary returns a concise one-line summary of an AWS SDK error for user-facing events, e.g.
// "operation error Auto Scaling: PutLifecycleHook failed with StatusCode: 403". It falls back to the
// full error string if the error isn't a recognizable AWS API/HTTP error. Use the raw error for logs
// (full detail) and this for Kubernetes events (concise).
func APIErrorSummary(err error) string {
	...
	if errors.As(err, &opErr) && errors.As(err, &respErr) {
		return fmt.Sprintf("operation error %s: %s failed with StatusCode: %d", opErr.Service(), opErr.Operation(), respErr.HTTPStatusCode())
	}
	return err.Error()
}
...
// match. Shared by the per-container hold and the cluster-level ensure.
