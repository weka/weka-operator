...
func (c *CliWekaService) ListSmbwClusterContainers(ctx context.Context) ([]int, error) {
	// weka smb cluster containers list --json
	// Returns HostId<N> strings similar to S3
	var hostIdStrings []string

	cmd := []string{
		"wekaauthcli", "smb", "cluster", "containers", "list", "--json",
	}
	err := c.RunJsonCmd(ctx, cmd, "ListSmbwClusterContainers", &hostIdStrings)
	if err != nil {
...
func (c *CliWekaService) JoinSmbwCluster(ctx context.Context, containerId int) error {
	...
	cmd := []string{
		"wekaauthcli", "smb", "cluster", "containers", "add", strconv.Itoa(containerId),
	}
...
func (c *CliWekaService) RemoveFromSmbwCluster(ctx context.Context, containerId int) error {
	...
	cmd := []string{
		"wekaauthcli", "smb", "cluster", "containers", "remove", strconv.Itoa(containerId),
	}
