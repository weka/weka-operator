...
func (api *ClusterAPI) updateClusterPassword(w rest.ResponseWriter, r *rest.Request) {
	...
	exec, err := podexec.NewExecInPod(api.k8sRestClient, api.k8sConfig, pod)
	if err != nil {
	...
	existingCredentials, err := api.getExistingCredentials(ctx, cluster)
	if err != nil {
		logger.Error(err, "Failed to get existing credentials")
		rest.Error(w, "Failed to get existing credentials", http.StatusInternalServerError)
		return
	}
	...
	logger.Info("Updating password", "username", username)
	password := credentials.Password
	command := []string{
		"bash", "-c",
		fmt.Sprintf(
			"WEKA_USERNAME=%s WEKA_PASSWORD=%s weka user passwd %s --username %s --current-password %s",
			existingCredentials.Data["username"],
			existingCredentials.Data["password"],
			password,
			username,
			existingCredentials.Data["password"],
		),
...
func (api *ClusterAPI) getPodForCluster(ctx context.Context, cluster *wekav1alpha1.WekaCluster) (*v1.Pod, error) {
	...
	matchLabels := client.MatchingLabels{"app.kubernetes.io/name": "SELECTOR-DISABLED-SHOULD-BE-OWNER"}
...
func (api *ClusterAPI) updateCredentialsSecret(ctx context.Context, cluster *wekav1alpha1.WekaCluster, password string) error {
	...
	if err := api.client.Update(ctx, secret); err != nil {
