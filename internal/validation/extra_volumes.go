...
func validateExtraVolumes(raw *runtime.RawExtension, mounts []corev1.VolumeMount, basePath *field.Path) field.ErrorList {
	...
	for i := range volumes {
		...
		// The pod factory rejects a sourceless volume too (applyExtraVolumes); catching it here
		// turns a per-container reconcile error into a rejected apply naming the entry.
		if v.VolumeSource == (corev1.VolumeSource{}) {
			errs = append(errs, field.Invalid(volumesPath.Index(i), v.Name,
				"has no volume source; check for a misspelled or missing source field"))
		}
