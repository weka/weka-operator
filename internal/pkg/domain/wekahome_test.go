...
func TestGetWekaHomeClientCacertSecret(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Pins that the env default still applies when inheritance is skipped: with an empty
			// default, the returned "" cannot tell "fell back" from "resolved to nothing".
			name:   "cross-namespace skip still falls back to the env default",
			client: &v1alpha1.WekaClient{ObjectMeta: metav1.ObjectMeta{Namespace: "clients-ns"}},
			targetCluster: &v1alpha1.WekaCluster{
				ObjectMeta: metav1.ObjectMeta{Namespace: "cluster-ns"},
				Spec:       v1alpha1.WekaClusterSpec{WekaHome: &v1alpha1.WekaHomeConfig{CacertSecret: "cluster-secret"}},
...
func TestWekaHomeAdditionalSecrets_PreservesOtherKeys(t *testing.T) {
	existing := map[string]string{"other": "keep", "wekahome-cacert": "old"}
	got := WekaHomeAdditionalSecrets(existing, "new")
	if got["other"] != "keep" || got["wekahome-cacert"] != "new" || existing["wekahome-cacert"] != "old" {
		t.Fatalf("set: got %v (input mutated: %v)", got, existing)
	}
	got = WekaHomeAdditionalSecrets(existing, "")
	if _, ok := got["wekahome-cacert"]; ok || got["other"] != "keep" {
		t.Fatalf("clear: got %v", got)
	}
	if got := WekaHomeAdditionalSecrets(nil, ""); got == nil || len(got) != 0 {
		t.Fatalf("nil input must yield an empty, non-nil map, got %v", got)
	}
}
