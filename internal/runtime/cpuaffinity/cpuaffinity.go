...
// DeriveNonDatapathCores computes the set of CPUs that are available for non-IONode
// (non-datapath) processes: all CPUs allowed for this pod minus the full-core IONode
// CPUs and all their HT siblings.
//
// procStatusPath is the path to a /proc/<pid>/status file containing Cpus_allowed_list;
// callers should pass "/proc/1/status".  The parameter is exposed for unit testing with
// a synthetic file.
//
// Mirrors the auto-derive branch of Python ensure_weka_container():
//
