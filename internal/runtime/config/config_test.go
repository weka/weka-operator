...
func TestFeatureFlagsFromBitmap(t *testing.T) {
	tests := []struct {
		...
		{
			// bit 12 = byte[1] bit 4 = 0x10 in byte[1]:
			// base64("AABA") decodes to bytes [0x00, 0x00, 0x00, 0x00] … but we need
			// 2-byte bitmap with byte[1]=0x10:
			// byte[0]=0x00, byte[1]=0x10 → base64 = "ABA="
			name:                         "bit 12 sets WekaManagesNonIonodeAffinity only",
			bitmap:                        "ABA=",
			wekaManagesNonIonodeAffinity: true,
		},
		{
			// Verify bit 12 NOT set when bitmap has bit 9 set but not bit 12.
			// byte[0]=0x00, byte[1]=0x02 → bit 9 set, bit 12 clear
			name:                 "bit 9 set does not set WekaManagesNonIonodeAffinity",
			bitmap:               "AAI=",
			ssdProxyIncludesDpdk: true,
		},
	}
