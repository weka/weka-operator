...
func runShutdownLoop(ctx context.Context, cfg *config.Config) error {
	...
	case modesNeedShutdownInstruction[cfg.Mode]:
		...
		graceful := shutdown.PollShutdownInstructions(ctx, cfg.PodID, bootID)
	...
	// This loop runs after watchCtx (derived from the parent ctx) is already cancelled in
	// the SIGTERM case, so it must NOT gate on ctx — it has to keep retrying until the
	// container is actually gone, mirroring Python's unbounded `while is_container_running()`
	// loop at weka_runtime.py:4524+. The stop command uses context.Background() and a 180s
	// timeout so each attempt completes even though the pod is terminating.
...
// Mirrors weka_runtime.py:4549 (commit a4024844): s3, drive, compute, nfs, smbw.
// Notably client is absent (always force-stops) and data-services was removed in
// commit a4024844 — data-services now force-stops as well.
...
func waitForFrontendDisconnect(ctx context.Context, containerName string) error {
	...
	for {
		...
		// Mirror Python weka_runtime.py:4546: "Frontend container '{container_name}' is still connected.
		// Waiting for it to disconnect... (timeout in {remaining:.0f} seconds)"
		logger.Info("Frontend container is still connected. Waiting for it to disconnect...",
			"container", containerName, "timeout_in_s", int(remaining.Seconds()))
		select {
		case <-ctx.Done():
			return ctx.Err()
