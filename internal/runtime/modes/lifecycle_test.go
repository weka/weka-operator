...
func TestComputeStopFlag(t *testing.T) {
	...
	// data-services was removed from gracefulEligibleModes in Python commit a4024844;
	// it must now force-stop in all cases.
	if got := computeStopFlag("data-services", false, false); got != "--force" {
		t.Errorf("data-services default = %q, want --force (data-services now force-stops)", got)
	}
