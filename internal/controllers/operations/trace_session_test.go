...
func TestTraceSessionDeploymentUpdatesServiceAccount(t *testing.T) {
	tests := []struct {
		name                      string
		desiredServiceAccountName string
	}{
		{
			name:                      "updates an existing service account",
			desiredServiceAccountName: "weka-runtime",
		},
		{
			name:                      "clears an existing service account",
			desiredServiceAccountName: "",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			existingDeployment := &appsv1.Deployment{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "trace-session-" + policy.Name,
					Namespace: policy.Namespace,
				},
			}
			existingDeployment.Spec.Template.Spec.ServiceAccountName = "old-service-account"

			kubeClient := fake.NewClientBuilder().WithScheme(scheme).WithObjects(existingDeployment).Build()
			mgr := &traceSessionTestManager{kubeClient: kubeClient, scheme: scheme}
			op := &MaintainTraceSession{
				cluster:          cluster,
				payload:          &weka.RemoteTracesSessionConfig{},
				mgr:              mgr,
				ownerRef:         policy,
				containerDetails: weka.WekaOwnerDetails{ServiceAccountName: tt.desiredServiceAccountName},
			}
			...
			var deployment appsv1.Deployment
			key := client.ObjectKey{Name: existingDeployment.Name, Namespace: existingDeployment.Namespace}
			if err := kubeClient.Get(context.Background(), key, &deployment); err != nil {
				t.Fatalf("get trace session deployment: %v", err)
			}
			if got := deployment.Spec.Template.Spec.ServiceAccountName; got != tt.desiredServiceAccountName {
				t.Errorf("ServiceAccountName = %q, want %q", got, tt.desiredServiceAccountName)
			}
		})
	}
}
