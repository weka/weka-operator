...
// lifecycleHookName is the fixed name of the AWS ASG EC2_INSTANCE_TERMINATING lifecycle hook the
// operator creates and drives to hold backend-node instances until their pods exit gracefully. It is a
// constant (not configurable): the operator both creates and manages the hook, so there is nothing to
// match.
const lifecycleHookName = "weka-drive-drain"
...
// releasedInstanceTTL bounds how long an instance stays marked "released" in-memory after the
// operator completes its termination lifecycle action, so the release path is not re-run — and
// CompleteLifecycleAction not re-called — on every reconcile while the instance lingers in
// Terminating:Wait behind another (non-operator) lifecycle hook. Stale entries are pruned on read.
const releasedInstanceTTL = time.Hour

var (
	// releasedInstancesMu guards releasedInstances.
	releasedInstancesMu sync.Mutex
	// releasedInstances records instances for which the operator has already completed its termination
	// lifecycle action (CONTINUE), keyed by instanceID, until releasedInstanceTTL elapses.
	releasedInstances = map[string]time.Time{}
)

// instanceReleased reports whether the operator already completed its termination lifecycle action
// for instanceID within releasedInstanceTTL. Stale entries are pruned on read.
func instanceReleased(instanceID string) bool {
	releasedInstancesMu.Lock()
	defer releasedInstancesMu.Unlock()
	t, ok := releasedInstances[instanceID]
	if !ok {
		return false
	}
	if time.Since(t) >= releasedInstanceTTL {
		delete(releasedInstances, instanceID)
		return false
	}
	return true
}

// markInstanceReleased records that the operator has completed its termination lifecycle action for
// instanceID, so subsequent reconciles skip the (idempotent) CompleteLifecycleAction call.
func markInstanceReleased(instanceID string) {
	releasedInstancesMu.Lock()
	defer releasedInstancesMu.Unlock()
	releasedInstances[instanceID] = time.Now()
}
...
func (r *containerReconcilerLoop) reconcileAwsTerminationLifecycle(ctx context.Context) error {
	...
	if instanceReleased(instanceID) {
		return nil
	}
	...
	markInstanceReleased(instanceID)
