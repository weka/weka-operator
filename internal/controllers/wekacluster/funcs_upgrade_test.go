...
func TestHandleSpecUpdates_PropagatesExtraVolumesToContainer(t *testing.T) {
	...
	wantVolumes := resources.NormalizeExtraVolumes(rawVolumes)
	if !resources.ExtraVolumesEqual(got.Spec.ExtraVolumes, wantVolumes) {
		t.Errorf("container.Spec.ExtraVolumes = %v, want %v", got.Spec.ExtraVolumes, wantVolumes)
	}
	if !reflect.DeepEqual(got.Spec.ExtraVolumeMounts, mounts) {
		t.Errorf("container.Spec.ExtraVolumeMounts = %v, want %v", got.Spec.ExtraVolumeMounts, mounts)
	}
}
