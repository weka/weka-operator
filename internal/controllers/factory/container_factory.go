...
func NewWekaContainerForWekaCluster(cluster *wekav1alpha1.WekaCluster,
	...
	additionalSecrets := domain.WekaHomeAdditionalSecrets(nil, wekahomeConfig.CacertSecret)
	...
	container := &wekav1alpha1.WekaContainer{
		...
		Spec: wekav1alpha1.WekaContainerSpec{
			...
			WekaContainerName:     strings.ReplaceAll(name, "-", "x"),
			Mode:                  role,
			NumCores:              numCores,
			...
			DriveTypesRatio:       template.DriveTypesRatio,
			...
			NonDatapathCoreIds:    cluster.GetNonDatapathCoreIdsForRole(role),
