...
// malformed bytes must be preserved rather than collapsed to nil, so a structurally invalid
// extraVolumes value still reaches the pod-build path's own error instead of silently vanishing.
...
// A misspelled source field survives the strict normalize as raw bytes, then the lenient decode in
// GetExtraVolumes drops it - leaving a Volume with no source at all. Without this guard the API
// server rejects every pod create with an opaque "must specify a volume type".
func TestApplyExtraVolumes_SourcelessVolumeErrors(t *testing.T) {
	_, err := createTestPod(t, weka.WekaContainerModeCompute, func(spec *weka.WekaContainerSpec) {
		spec.ExtraVolumes = &runtime.RawExtension{Raw: []byte(`[{"name":"v1","secretname":{"secretName":"ca"}}]`)}
	})
	if err == nil {
		t.Fatal("expected applyExtraVolumes to reject a volume with no source, got nil")
	}
	if !strings.Contains(err.Error(), "no volume source") {
		t.Errorf("unexpected error %v, want it to name the missing volume source", err)
	}
}

// Strict decoding treats an unknown field as a decode failure (same branch as
// MalformedInputSurvives above), so it is preserved as-is instead of silently dropped through a
// lenient re-marshal — the CRD's PreserveUnknownFields means the API server accepted that field
// legitimately.
...
// recognize. The mount-path check catches a case the name-only check cannot: a new mount path
// added without a corresponding new volume name.
...
// TestIsReservedMountPath_AncestorsAndDescendants: extra mounts are appended after the operator's,
// so a mount above a reserved path shadows it just like a mount on or under it.
func TestIsReservedMountPath_AncestorsAndDescendants(t *testing.T) {
	for p, want := range map[string]bool{
		...
		"/usr/bin":         true,
		"/etc/ssl/certs":   false,
		"/opt/weka-extras": false,
		"/tmp":             false,
	} {
		if got := IsReservedMountPath(p); got != want {
			t.Errorf("IsReservedMountPath(%q) = %v, want %v", p, got, want)
		}
	}
}

// Normalization is what byte-equality comparison rests on: a spec reformatted with different
// whitespace or key order must produce identical bytes, or every reconcile would see a change.
func TestNormalizeExtraVolumes_WhitespaceAndKeyOrderAreIgnored(t *testing.T) {
	compact := &runtime.RawExtension{Raw: []byte(`[{"name":"v1","emptyDir":{}}]`)}
	...
	got1 := NormalizeExtraVolumes(compact)
	got2 := NormalizeExtraVolumes(reordered)
	if got1 == nil || got2 == nil {
		t.Fatal("expected both to normalize to a non-nil value")
	}
	if !ExtraVolumesEqual(got1, got2) {
		t.Errorf("normalized form changed under whitespace/key-order variation: %s != %s", got1.Raw, got2.Raw)
	}
}

func TestExtraVolumeMountsEqual_NilAndEmptyAreEqual(t *testing.T) {
	if !ExtraVolumeMountsEqual(nil, []corev1.VolumeMount{}) {
		t.Error("nil and empty must compare equal, or an unset spec reads as a change every reconcile")
	}
	if ExtraVolumeMountsEqual(nil, []corev1.VolumeMount{{Name: "v1", MountPath: "/mnt"}}) {
		t.Error("nil and a non-empty slice must not compare equal")
	}
}
