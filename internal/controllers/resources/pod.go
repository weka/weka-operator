...
func (f *PodFactory) Create(ctx context.Context, podImage *string) (*corev1.Pod, error) {
			...
			Containers: []corev1.Container{
				{
					...
					Env: []corev1.EnvVar{
						...
						{
							Name:  "LOG_LEVEL",
							Value: fmt.Sprint(config.Config.Logging.Level),
						},
						{
							Name:  "LOG_FORMAT",
							Value: config.Config.Logging.Format,
						},
					},
				},
			},
	...
	if config.Config.NetnsEnabled {
		hostsideSharedNetns := f.nodeInfo.GetHostsideSharedNetnsPath()
		pod.Spec.Containers[0].VolumeMounts = append(pod.Spec.Containers[0].VolumeMounts, corev1.VolumeMount{
			Name:             "host-shared-netns",
			MountPath:        inPodHostBinds + "/shared-netns",
