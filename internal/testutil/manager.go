...
func (c *stubClient) List(ctx context.Context, list client.ObjectList, opts ...client.ListOption) error {
	...
	for _, object := range objects {
		...
		itemsField.Set(reflect.Append(itemsField, objectValue))
	}
...
func (c *stubClient) Create(ctx context.Context, obj client.Object, opts ...client.CreateOption) error {
	...
	objects, ok := c.state[objectType]
	if !ok {
		c.state[objectType] = make(map[types.NamespacedName]client.Object)
		objects = c.state[objectType]
	}
	...
	objects[key] = obj.DeepCopyObject().(client.Object)
	return nil
}
...
// StatuClient
func (c *stubClient) Status() client.StatusWriter { return &stubStatusWriter{client: c} }
...
func (w *stubStatusWriter) Update(ctx context.Context, obj client.Object, opts ...client.SubResourceUpdateOption) error {
	...
	// Get the objects map for this type
	objects, ok := w.client.state[objectType]
	if !ok {
	...
	existingObj, ok := objects[key]
	if !ok {
		return fmt.Errorf("object %s not found", key)
	}

	// Update the status using reflection
	existingObjValue := reflect.ValueOf(existingObj).Elem()
	objValue := reflect.ValueOf(obj).Elem()

	statusField := existingObjValue.FieldByName("Status")
	if !statusField.IsValid() {
		return fmt.Errorf("status field not found in object")
	}

	newStatusField := objValue.FieldByName("Status")
	if !newStatusField.IsValid() {
		return fmt.Errorf("status field not found in new object")
	}

	// Update the status
	statusField.Set(newStatusField)

	// Save back to the original instance
...
func (m *stubManager) AddMetricsServerExtraHandler(path string, handler http.Handler) error {
	return nil
}
