...
The operator maintains a ConfigMap, Deployment and Service for the management proxy, plus an
Ingress when `MANAGEMENT_PROXY_INGRESS_BASE_DOMAIN` is set (the chart default is empty, so a default
install gets no Ingress).
Implementation: `internal/controllers/wekacluster/funcs_management_proxy.go` and
`internal/controllers/wekacluster/funcs_management_service.go` (endpoint selection). One proxy per WekaCluster, fronting up to
...
`EnsureManagementProxy` returns `nil` when `selectActiveContainersForManagement` finds no container
(base port not yet allocated, or every drive/compute container transiently non-operational). It only
logs `No active containers found for management proxy, skipping`: no ConfigMap write, no Deployment
reconcile, no event, previous proxy config left in place. Check for that line when the proxy looks stale.

Tunables come from `MANAGEMENT_PROXY_*` env, gathered into `managementProxySettings`. Defaults live
in `internal/config/env.go`, not the chart: `manager.yaml` omits `REPLICAS`, `HEALTHY_PANIC_THRESHOLD`
and `ADMIN_BIND_ADDRESS` when unset so there is one place a default is written. `adminBindAddress` defaults to loopback
under `hostNetwork` (admin is unauthenticated, includes `/quitquitquit`). All are operator-global —
no per-WekaCluster override — but only the ones that reach the bootstrap or pod spec
(`healthyPanicThreshold`, `adminBindAddress`, `hostNetwork`) re-roll every managed cluster's proxy
pods. `replicas` only scales each Deployment; `INGRESS_*` only updates each Ingress.
...
keeping one ready throughout. Placement constraint: under `hostNetwork`
...
and updates filesystem EDS without rolling proxy pods. Membership therefore still reacts to transient state.

## healthyPanicThreshold and proxy start

Defaults to 50. It fails open: a false negative from the single
...
- `/ready` gates it wherever `adminProbeHost` can reach admin, which is every default. The readiness check can return 200
...
The target is the node's loopback, not a separate pod loopback. Replicas sharing the same
node and admin port collide at bind time; scheduling does not prevent this placement
(see the host-port constraint above).
