...
strictly, rejecting unknown fields so a typo like `secretname:` for
`secretName:` is caught immediately instead of being silently dropped — a
dropped source field leaves the volume with no source at all. An entry with
no source is rejected as well, whether it lost one to a typo or never had
one. Both validators are `Error`
...
otherwise well-formed volume (like `secretname:` above) are not re-checked
there, because `encoding/json` ignores unrecognized fields by default; only
admission's `DisallowUnknownFields` decoding catches those. The pod factory
does still refuse a volume left with no source, so the typo surfaces as a
reconcile error on each WekaContainer rather than a silently broken pod.
...
The operator reserves certain volume names and mount paths for its own use inside weka/client pods. An `extraVolumes` entry or
...
**Reserved mount paths** (`ReservedMountPaths`) — a mount cannot land on,
under, or above any of these. "Above" matters because extra mounts are added
after the operator's own, so a mount at `/opt` or `/` would shadow `/opt/weka`
just as a mount on it would:
...
The last four are files rather than directories; the same on-or-under rule
applies, they simply have nothing under them.
...
The operator's own Deployment (the Helm `manager.extraVolumes` surface) has a
much smaller, separate reservation: the volume name `tmpdir` and the mount path
`/tmp`, enforced by a `{{ fail ... }}` in the Helm template itself. `/tmp` is
the `tmpdir` emptyDir where the webhook writes its serving certificates
(`WEBHOOK_CERT_DIR`, default `/tmp/k8s-webhook-server/serving-certs`), so a
mount on, under, or above it stops admission from serving — and with
`failurePolicy: Fail` that rejects every WekaCluster and WekaClient apply. The
API server only catches an exact duplicate of the name or the path; a mount at
`/tmp/k8s-webhook-server` or at `/` it would accept, which is why the template
checks rather than relying on apply-time validation.
