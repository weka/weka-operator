...
- `status.status` field should reach "Ready" state, can be looked up by `kubectl get -o wide CLUSTER_NAME -n NAMESPACE`, or by json representations of `status.status` field
